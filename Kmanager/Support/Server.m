...
{
    [self request:@"http://svkmanager.herokuapp.com/api/user/login"
           method:@"POST"
...
- (void)logoutWithUsername:(NSString*)idnv token:(NSString*)token
               completion:(void (^)(NSError* error, NSDictionary* data))completion
{
    NSString *url = [NSString stringWithFormat:@"http://svkmanager.herokuapp.com/api/user/logout?id=%@", idnv];
    [self requestGET:url
            paras:@{}
       completion:completion];
}
...
- (void)deleteData:(NSString*)url completion:(void (^)(NSError* error, NSDictionary* data))completion;
{
