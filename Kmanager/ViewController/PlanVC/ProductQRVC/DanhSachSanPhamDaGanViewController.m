...
-(void)requestPlan
{
    NSString *url = [NSString stringWithFormat:@"http://svkmanager.herokuapp.com/api/schemeProductDetail?idlogin=%@", [[NSUserDefaults standardUserDefaults] objectForKey:@"idnhanvien"]];
...
-(void)requestGanDeQR
{
    NSString *url = [NSString stringWithFormat:@"http://svkmanager.herokuapp.com/api/schemeProductDetail?idschemeproductdetail=%@&qrcode=%@", idProduct,qrCode];
    [[Server sharedServer] putData:@"http://svkmanager.herokuapp.com/api/customer" param:@{} completion:^(NSError *error, NSDictionary *data){
        if (error == nil) {
            NSInteger status = [[data objectForKey:@"status"] integerValue];
            if (status == true) {
                NSString *msg = [NSString stringWithFormat:@"%@", [data objectForKey:@"msg"]];
