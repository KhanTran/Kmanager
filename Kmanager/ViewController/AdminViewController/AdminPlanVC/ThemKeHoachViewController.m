...
//        [self.btnAddEdit setTitle:@"Sửa"];
        ...
        idKeHoach = [NSString stringWithFormat:@"%@", [_dicKehoach objectForKey:@"idplan"]];
...
//        return;
//    }
    if (_lblCustomer.text.length > 0 && _tfFromDate.text.length > 0 && _tfToDate.text.length > 0 && _tfStaff.text.length > 0 && _tfPlan.text.length > 0 && arrSPSend.count > 0) {
        if (_viewMode == 2) {
            [self sendDataEdit];
        }
        else
        ...
        UIAlertController* c = [UIAlertController alertControllerWithTitle:@"" message:@"Bạn cần nhập đầy đủ thông tin trước khi lưu" preferredStyle:UIAlertControllerStyleAlert];
        UIAlertAction* okAct = [UIAlertAction actionWithTitle:@"Xong" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
...
- (NSDictionary*) getJson
{
    ...
    if (_viewMode == 2) {
        [postDic setValue:idKeHoach forKey:@"_id"];
    }
...
-(void) sendData
{
    ...
    [[Server sharedServer] postData:[NSString stringWithFormat:@"%@scheme",ServerApi] param:postDic completion:^(NSError *error, NSDictionary *data){
        ...
        if (error == nil) {
            ...
            if (status == true) {
                UIAlertController* c = [UIAlertController alertControllerWithTitle:@"" message:@"Thêm mới kế hoạch thành công" preferredStyle:UIAlertControllerStyleAlert];
...
-(void) sendDataEdit
{
    ...
    [[Server sharedServer] putData:[NSString stringWithFormat:@"%@scheme",ServerApi] param:postDic completion:^(NSError *error, NSDictionary *data){
        [[Server sharedServer] hideAnimation:self.view];
        ...
        if (error == nil) {
            ...
            if (status == true) {
                UIAlertController* c = [UIAlertController alertControllerWithTitle:@"" message:@"Sửa kế hoạch thành công" preferredStyle:UIAlertControllerStyleAlert];
                UIAlertAction* okAct = [UIAlertAction actionWithTitle:@"OK" style:UIAlertActionStyleCancel handler:^(UIAlertAction * _Nonnull action) {
                    [[NSNotificationCenter defaultCenter] postNotificationName:@"ReloadData" object:nil userInfo:nil];
