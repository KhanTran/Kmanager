...
-(void)requestCustomer
{
    ...
    if ( _viewMode == 4) {
        url = [NSString stringWithFormat:@"%@customer/all?idlogin=%@", ServerApi,[[NSUserDefaults standardUserDefaults] objectForKey:@"idnhanvien"]];
    }
    else if (_viewMode == 3)
    {
        url = [NSString stringWithFormat:@"%@customer?idlogin=%@", ServerApi,_idNhanVien];
    }
...
-(void)btnXoaKHAction:(id)sender {
    ...
    NSString *url = [NSString stringWithFormat:@"%@customer?idlogin=%@&id=%@", ServerApi,[[NSUserDefaults standardUserDefaults] objectForKey:@"idnhanvien"],idKhachHang];
    [[Server sharedServer] showAnimation:self.view];
    [[Server sharedServer] deleteData:url param:@{} completion:^(NSError *error, NSDictionary *data) {
        [[Server sharedServer] hideAnimation:self.view];
